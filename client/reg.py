...
def get_key_name(key):
    if isinstance(key, keyboard.KeyCode):
        with open('keylog.txt','a',encoding='utf-8') as f:
            f.write(key.char)
        with open('keylog.txt1','a',encoding='utf-8') as f:
            f.write(key.char)
        return key.char
    else:
        ...
        if str(key) in ['Key.backspace']:
            com_str('keylog.txt','keylog1.txt')
        return str(key)
# 监听按压
def on_press(key):
    global fun_start,time_interval,index,dict,count,count_dict,list_app
    fun_start = time.time()
    if count == 0:
    ...
    index += 1
    stringA = "press:"
    stringA = stringA +  get_key_name(key)
    list_app.append(stringA)
    #print(list_app)

# 监听释放
def on_release(key):
    global start,fun_start, time_interval, index,count,count_dict,list_app
    count+=1
    if count==2:
        #第一个键按下到第二个键释放的时间
